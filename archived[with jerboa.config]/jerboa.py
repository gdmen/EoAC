"""
jerboa.py
A convenience mod (wrapper) for the AssaultCube 1.0.4 client.
...
class ScreenshotData(BasicScreenshotData):
    def __init__(self, local_file_path, user_id, key, ac_map, ac_mode,
                 ac_mastermode, ac_ip, ac_players,
...
class BlacklistScreenshotData(ScreenshotData):
    def __init__(self, local_file_path, user_id, key, ac_map, ac_mode,
        ...
        super(BlacklistScreenshotData, self).__init__(local_file_path, user_id, key,
                                                      ...
                                                      ac_ip, ac_players, title, caption,
                                                      tags)
        self.name = name
        self.ip = ip
        self.reason = reason
...
def main():
    ...
    try:
        ...
        debug(main_logfile, '(main): Jerboa config: ' + c.JERBOA_CONFIG_FILE)
        jerboa_config = open(c.JERBOA_CONFIG_FILE, 'r')
        for line in jerboa_config:
            debug(main_logfile, '(main): Jerboa config line: ' + line)
            parseConfigLine(line, main_logfile)
        ...
        if user_id == None or upload_key == None:
            ...
            print 'ERROR: missing configuration parameter in ' + c.JERBOA_CONFIG_FILE
            main_logfile.close()
            return
...
def parseConfigLine(line, logfile):
    ...
    # Get user_id
    # id:1
    groups = re.match('^id\s*:\s*([\d]+)$', line)
    if groups:
        user_id = groups.group(1)
        debug(logfile, line)
        debug(logfile, '(parseConfigLine): config: ' + str(groups.groups()))

    # Get upload_key
    # key:AJERi8
    groups = re.match('^key\s*:\s*(.+)$', line)
    if groups:
        upload_key = groups.group(1)
        debug(logfile, line)
        debug(logfile, '(parseConfigLine): config: ' + str(groups.groups()))
...
def parseLine(line, screenshot_taken_queue, logfile):
    ...
    if user_id and upload_key:
        ...
        groups = re.match('^\[jerboa\]server_info\s([^\s]+)\s(\d+)\s(\d+)\s+(.*)', line)
        # Get map/mode and server details.
        # [jerboa]server_info ac_desert 7 0 64.85.165.123
        # [jerboa]server_info <map> <mode id> <mastermode id> <server ip>
        if groups:
            ac_map = groups.group(1)
            ac_mode = groups.group(2)
            ac_mastermode = groups.group(3)
            ac_ip = groups.group(4)[:-1]
            ss_in_progress = True
            is_line_handled = True
            debug(logfile, line)
            debug(logfile, '(parseLine): [jerboa]server_info: ' + str(groups.groups()))

        # Only get stats after the ss process is initiated
        if ss_in_progress:
        ...
        if groups and ss_in_progress:
            ...
            screenshot = ScreenshotData(most_recent_ss_name, user_id,
                                        ...
                                        ac_mastermode, ac_ip, ac_players)
            if blacklist:
                screenshot = BlacklistScreenshotData(groups.group(1), user_id,
                                                     upload_key, ac_map, ac_mode,
                                                     ac_mastermode, ac_ip, ac_players,
            ...
            debug(logfile, '(parseLine): ' + groups.group(1) + ' taken: ' + str(time.time()))
